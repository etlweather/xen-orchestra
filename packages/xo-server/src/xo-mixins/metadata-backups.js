...
const METADATA_BACKUP_JOB_PROPERTIES = new Set([
  'id',
  'name',
  'pools',
  'remotes',
...
export default class metadataBackup {
  ...
  async createMetadataBackupJob(
    ...
    await asyncMap(schedules, schedule =>
      app.createSchedule({ ...schedule, jobId: job.id })
    )

    return job
  }

  async editMetadataBackupJob(
    props: MetadataBackupJob
  ): Promise<MetadataBackupJob> {
    const app = this._app

    const job = await app.getJob(props.id)
    if (job.type !== METADATA_BACKUP_JOB_TYPE) {
      props.type = METADATA_BACKUP_JOB_TYPE
      for (const key in job) {
        if (!METADATA_BACKUP_JOB_PROPERTIES.has(key)) {
          props[key] = null
        }
      }
    }

    return app.updateJob(props)
  }
